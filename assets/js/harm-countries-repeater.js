...
const init = function init() {
  const container = document.getElementById('harm-claim-country-repeater');
  ...
  addAnotherLink.appendChild(document.createTextNode('Add another country'));
  // eslint-disable-next-line no-script-url
  addAnotherLink.href = 'javascript:void(0)';
  addAnotherLink.onclick = function onclick(e) {
    e.preventDefault();
  ...
  fields.forEach(function eachField(field, n) {
    ...
    removeLink.className = 'remove-harm-claim-country';
    removeLink.appendChild(document.createTextNode('Remove'));
    removeLink.onclick = function onClick(e) {
      e.preventDefault();
