...
const maxFileSize = config.applicationUploads.maxFileSize;
const allowedMimeTypes = config.applicationUploads.allowedMimeTypes;
const mandatoryColumns = config.applicationUploads.mandatoryColumns;
const applicantColumns = config.applicationUploads.applicantColumns;
const referenceColumns = config.applicationUploads.referenceColumns;
...
module.exports = name => superclass => class extends superclass {
  getValues(req, res, next) {
    ...
    const caseworkerEmail = isLocalhost ? config.login.skipEmail : req.headers['x-auth-email'];

    req.sessionModel.set('caseworker-email', caseworkerEmail);
  ...
  validateField(key, req) {
    ...
    if (fileUpload) {
      ...
      if (invalidSize || invalidMimetype) {
        return new this.ValidationError('bulk-upload-claimants', {
